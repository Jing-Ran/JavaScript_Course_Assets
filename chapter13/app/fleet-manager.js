...
export class FleetManager {
  constructor(fleetData) {
    ...
    let vehicleHandlers = {
      motorbikes: (vehicle) => {
        this.fleet.push(new Motorbike(vehicle.license, vehicle.capacity,
          vehicle.location, (vehicle.secureDocumentStorage ||
            undefined)));
      },
      vans: (vehicle) => {
  ...
  filterByCapacity(capacity) {
    return this.fleet.filter(vehicle => parseInt(vehicle.capacity.split('ft2')[
      0], 10) >= capacity);
  }

  filterByFeature(feature) {
    return this.fleet.filter(vehicle => vehicle[feature] !== false && vehicle[
      feature] !== undefined);
  }
};