        ...
        function middleware(req, res, next) {
          ...
          let urls = {
            '/response.json': __dirname +
              '/chapter8/response.json',
            '/signin': __dirname + '/chapter8/loginresponse.json',
            '/get_request': __dirname +
              '/chapter8/getresponse.json',
            '/get_request?test_data': __dirname +
              '/chapter8/getresponse.json',
            '/create_account': __dirname +
              '/chapter8/createaccount.json'
          };
          let match = false;
          ...
          if (req.url === '/upload') {
            ...
            busboy.on('file', function(fieldname, file, filename,
              encoding, mimetype) {
              let destinationFile = path.join(__dirname +
...
gulp.task('setup', function() {
  ...
  require('child_process').exec(
    'fsutil file createnew uploadFile 1073741824',
    function(err) {
